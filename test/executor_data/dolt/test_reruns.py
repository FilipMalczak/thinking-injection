...
from thinking_executor_data.dolt.sqlalchemy.storage import DoltStorage, DoltRepository, SQLFilter
...
#todo non-happy cases; ditto terminus
...
def expect_skipped_step():
    assert False, "This step should be skipped as executed previously"

# these tests have a large overlap with "normal" executor tests, but we should still do that
# just to be sure that introducing data layer doesn't break anything
#
# they also have a lot of overlap (when it comes to asserting) with first runs, but it won't hurt neither
...
def just_create():
    ...
    with ctx.lifecycle() as idx:
        ...
        def top_level():
            ...
            assert_repo_is_empty(repo)
            assert_doesnt_exists(repo, e.id_)
            assert_cannot_be_found(repo, DumbEntity.txt == current_case_name())
            assert_cannot_be_saved(repo, DumbEntity(txt="should fail"))
...
for deleter in [
    ...
    NamedLambda("by where txt=...", lambda r, e: r.delete().where(DumbEntity.txt == current_case_name()+"1"))
]:
    ...
    def create_two_then_delete(d):
        ...
        with ctx.lifecycle() as idx:
            ...
            def top_level():
                def create_step():
                    ...
                    entities.extend(repo.save(e1, e2))
                    assert_repo_size_is(repo, 2)
                    assert_exists(repo, entities[0].id_)
                    assert_exists(repo, entities[1].id_)
                    assert_can_be_found(repo, DumbEntity.txt == e1.txt)
                    assert_can_be_found(repo, DumbEntity.txt == e2.txt)
        ...
        with ctx.lifecycle() as idx:
            ...
            def top_level():
                ...
                assert_repo_is_empty(repo)
                assert_doesnt_exists(repo, entities[0].id_)
                assert_doesnt_exists(repo, entities[1].id_)
                assert_cannot_be_found(repo, DumbEntity.txt == e1.txt)
                assert_cannot_be_found(repo, DumbEntity.txt == e2.txt)
                assert_cannot_be_saved(repo, DumbEntity(txt="should fail"))
