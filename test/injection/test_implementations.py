...
def test_concrete_hierarchy_with_specific_primaries():
    impls = Implementations.build({ConcreteParent2, ConcreteChild21, ConcreteChild22, ConcreteGrandchild211, ConcreteGrandchild212})
    expected = Implementations({
...
def no_impl_for_interface():
    impls = Implementations.build([TheInterface])
    expected= Implementations({TheInterface: ImplementationDetails(frozenset(), None)})
    assert expected == impls

@case
def single_impl_for_interface():
    impls = Implementations.build([TheInterface, Impl1])
    expected = Implementations({
...
def multiple_impls_for_interface():
    impls = Implementations.build([TheInterface, Impl1, Impl2, Impl3, SubInterface1, SubImpl4])
    expected = Implementations({
...
def no_impls_for_interface_w_primary():
    impls = Implementations.build([AnotherInterface])
    expected = Implementations({
...
def one_interface_impl_that_isnt_primary():
    impls = Implementations.build([AnotherInterface, AnotherImpl1])
    expected = Implementations({
...
def one_interface_impl_that_is_primary():
    impls = Implementations.build([AnotherInterface, AnotherImpl2])
    expected = Implementations({
...
def interface_with_multiple_impls_and_primary():
    impls = Implementations.build([AnotherInterface, AnotherImpl1, AnotherImpl2])
    expected = Implementations({
